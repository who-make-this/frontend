        ...
        <div className="fixed inset-0 backdrop-blur-[10px] z-50 flex flex-col pt-3 animate-fade-in">
            ...
            <div className="flex-grow overflow-y-auto hide-scrollbar">
                {loading ? (
                    ...
                    <div className="flex justify-center items-center h-full"><p className="text-white text-center">선택할 수 있는 이미지가 없습니다.</p></div>
                ) : (
                    <div className="grid grid-cols-3 gap-1">
                        {images.map((imgSrc, index) => (
                            <div key={index} className="aspect-square bg-gray-700 overflow-hidden" onClick={() => onSelectImage(imgSrc)}>
                                <img src={imgSrc} alt={`Gallery image ${index + 1}`} className="w-full h-full object-cover cursor-pointer transition-transform duration-300" />
                            </div>
                        ))}
                    </div>
                )}
...
const formatDate = (iso) => new Date(iso).toLocaleDateString('ko-KR').replace(/\./g, '/').slice(0, -1);
