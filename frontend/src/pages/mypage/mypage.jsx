...
const MyPageLoading = () => (
    <div className="absolute inset-0 z-40 flex items-center justify-center bg-white/50 backdrop-blur-sm">
        <div className="w-12 h-12 border-4 border-dashed rounded-full animate-spin border-gray-800"></div>
    </div>
);
...
export default function MyPage() {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                const combinedData = {
                    ...userResponse.data, // /users/me 응답 데이터
                    mileage: mileageResponse.data.remaining // /mileage/me 응답 데이터
                };
    ...
    const MainTypeIcon = typeIcons[userData.mainType];
    const typeGradientColors = { '먹보형': 'to-[#D19B98]', '모험형': 'to-[#889F69]', '감성형': 'to-[#A792B9]' };
    const toColorClass = typeGradientColors[userData.mainType] || 'to-[#D19B98]';
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-gray-100">
            <div className="w-[375px] h-[812px] bg-white shadow-sm relative overflow-hidden">
                ...
                <div className="absolute top-[102px] left-1/2 -translate-x-1/2 w-[355px] h-[655px] px-8 py-10 flex flex-col">
                    ...
                    <div className="mt-[0px]">
                        ...
                        <div className="relative mt-[55px] h-12">
                            <div className="absolute left-[11px]"><p className="text-[15px] font-[400] text-[#2B2B2B80]">Place</p><p className="text-[16px] font-semibold">{userData.lastMarket}</p></div>
                            <div className="absolute right-3"><p className="text-[15px] font-[400] text-[#2B2B2B80]">Start Date</p><p className="text-[16px] font-semibold text-right">{userData.createdAt}</p></div>
                        </div>
                        <div className="relative h-12 mt-6">
                            <div className="absolute left-[11px]"><p className="text-[15px] font-[400] text-[#2B2B2B80]">Explorations</p><p className="text-[16px] font-semibold">{userData.explorationCount}</p></div>
                            <div className="absolute right-[48px]"><p className="text-[15px] font-[400] text-[#2B2B2B80]">Cleared</p><p className="text-[16px] font-semibold text-start">{userData.clearedCount}</p></div>
                        </div>
                    </div>
                ...
                {isModalOpen && <CouponModal onClose={handleCloseModal} isClosing={isClosing} currentMileage={userData.mileage} />}
            </div>
        </div>
    );
}
